...
# bundled template (from package.json) so the bundled comparison reports "up to date".
echo "{\"version\":\"${BUNDLED_VERSION}\",\"installedPacks\":[\"software-development\"],\"installedAt\":\"2026-01-01T00:00:00.000Z\",\"optionalSelections\":[\"scripts/build.sh\"]}" > "$TEMP_DIR/.codice-version"

log_info "Pre-populated project with version ${BUNDLED_VERSION} (v2.0 format) files"
