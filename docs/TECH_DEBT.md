...
| **TD-V2-7** | Coverage gate below 95% | 2-4h | Low | **OPEN (detected FEV-23, 2026-08-07):** `just coverage-check 95` reports 93.78% overall (2908/3101 lines) — the 95% CI gate has been failing since FEV-17→22. Production `src/` alone is 99.48% (2115/2126); the gap lives in non-src files counted by `bun test tests/ --coverage`: test helpers (`tests/unit/domain/helpers/agentFrontmatterValidator.ts` 85 lines @ 60%), template plugin source (`template/obligatorio/core/.opencode/plugins/src/{directoryScanner,autoDiscovery}.ts` 58 lines), dev scripts (`scripts/{distribute-agents,reformat-agent}.ts` 28 lines), packaging/setup helpers (~11 lines), and Windows-only branches in `src/cli/parse-args.ts` (lines 34-41, 85 — win32 system dirs + drive roots, untestable on Linux CI). Options: exclude non-installer files from the metric (like `skills/**` already is), or add plugin/dev-script coverage. **Deferred** — FEV-23 closed v2.0.0 with documented real numbers; revisit for v2.1.0. |
