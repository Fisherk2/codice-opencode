...
Every agent file follows the same structure: YAML frontmatter, markdown body, and a `## COMPOSITION` block at the end. The complete specification — including field mapping, canonical permission blocks, and transformation rules — is documented in [specs/spec-agent-format-v2.md](../specs/spec-agent-format-v2.md). Primary agents additionally carry a protocol section between `### RULES` and `## KNOWLEDGE` — `## DELEGATION PROTOCOL` when they can invoke `task()`, `## SKILL ANALYSIS PROTOCOL` when they cannot (see §8 of the spec).
