...
- **Invoke via:** `/webperf` (dedicated performance audit command, mapped to Mictlantecuhtli, SDD phase: review). Not included in `/ship` fan-out — performance audits apply to web applications only, not to utility libraries or CLI tools, so adding it to a global pre-launch fan-out would create noise in non-web projects.
